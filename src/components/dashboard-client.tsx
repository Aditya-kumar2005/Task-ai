...
import { getAISubtaskSuggestions, getAIDetailedSteps } from '@/app/actions';
...
const TaskForm = dynamic(() => import('@/components/task-form').then(mod => mod.TaskForm), {
  loading: () => <Skeleton className="h-[430px] w-full rounded-lg" />,
  ssr: false
});
...
const SubtaskSuggestionDialog = dynamic(() => import('@/components/subtask-suggestion-dialog').then(mod => mod.SubtaskSuggestionDialog), { ssr: false });
...
export function DashboardClient() {
  ...
  return (
    <>
      ...
      <Suspense fallback={<Skeleton className="h-48 w-full rounded-lg mt-8" />}>
        <TaskList
          tasks={tasks}
